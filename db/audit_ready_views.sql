-- Canonical for CLI: supabase/migrations/20250423140001_parable_ledger_audit_views.sql
-- =============================================================================
-- PARABLE Ledger — IRS 990 / 990-T prep views & UBI fund seed
-- Run AFTER db/parable_ledger_schema.sql, BEFORE db/update_ledger_schema.sql (which replaces v_transactions_compliance).
-- =============================================================================
-- Not legal or tax advice. 990-T filing obligations depend on facts, gross vs net,
-- and current regulations — use counsel before relying on filing_indicator text.
-- =============================================================================

-- Dedicated book fund for unrelated-business-lane receipts (990-T workflow).
...
-- -----------------------------------------------------------------------------
-- Compliance projection (maps schema columns → reporting flags)
-- -----------------------------------------------------------------------------
...
COMMENT ON VIEW parable_ledger.v_transactions_compliance IS 'UBI vs mission lane derived from contribution_nature + metadata.tax_lane.';

-- -----------------------------------------------------------------------------
-- Form 990-T style annual rollup (gross UBI-class receipts in ledger)
-- -----------------------------------------------------------------------------
...
COMMENT ON VIEW parable_ledger.rpt_irs_990t_summary IS 'Aggregates UBI-class rows by calendar year; confirm net UBIT and exceptions with a CPA.';

-- -----------------------------------------------------------------------------
-- Donors who may need written acknowledgment / receipt discipline ($250+ rule)
-- -----------------------------------------------------------------------------
...
COMMENT ON VIEW parable_ledger.rpt_high_value_donor_candidates IS 'Per-year gifts >= $250 by donor_ref in metadata; generate PDF ack separately.';
