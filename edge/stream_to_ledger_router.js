/**
 * PARABLE Ledger — stream / gaming → ledger router (Node / tooling copy)
 * Deployed canonical file: supabase/functions/stream-to-ledger/stream_to_ledger_router.js
 * Keep both files identical when changing classification rules.
 */
...
export async function handleStreamRevenue(supabase, payload) {
  ...
  if (!fundId) {
    ...
    if (!fundRow?.id) {
      throw new Error(
        `No ministry_funds for tenant_id=${tenantId} fund_code=${fundCode}. Run genesis provisioning or pass fundId.`,
      );
    }
    fundId = fundRow.id;
  }
