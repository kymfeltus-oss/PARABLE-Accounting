-- Atomic expense recording write path via SECURITY DEFINER RPC.
...
begin
  ...
  select
    count(*),
    coalesce(sum(expense_line.amount), 0)
  into
    line_count,
    line_total
  from public.expense_lines as expense_line
  where expense_line.expense_id = target_expense_id;
  ...
  if exists (
    select 1
    from public.expense_lines as expense_line
    inner join public.accounts as debit_account
      on debit_account.id = expense_line.account_id
    where expense_line.expense_id = target_expense_id
      and (
        debit_account.organization_id <> target_organization_id
        or debit_account.account_type <> 'expense'
        or debit_account.status <> 'active'
        or debit_account.is_posting is not true
      )
  ) then
  ...
  if exists (
    select 1
    from public.expense_lines as expense_line
    inner join public.funds as line_fund
      on line_fund.id = expense_line.fund_id
    where expense_line.expense_id = target_expense_id
      and expense_line.fund_id is not null
      and (
        line_fund.organization_id <> target_organization_id
        or line_fund.status <> 'active'
      )
  ) then
  ...
  where accounting_period.organization_id = target_organization_id
    and current_expense.expense_date >= accounting_period.start_date
    and current_expense.expense_date <= accounting_period.end_date;
  ...
  for fund_bucket in
    select
      bucket.fund_id,
      sum(bucket.amount) as bucket_total
    from public.expense_lines as bucket
    where bucket.expense_id = target_expense_id
    group by bucket.fund_id
    order by bucket.fund_id nulls last
  loop
    ...
    values (
      ...
      current_expense.description,
      0,
      fund_bucket.bucket_total
    );
  end loop;

  select
    coalesce(sum(journal_line.debit_amount), 0),
    coalesce(sum(journal_line.credit_amount), 0)
  into
    total_debits,
    total_credits
  from public.journal_entry_lines as journal_line
  where journal_line.journal_entry_id = created_journal_entry_id;
  ...
  if exists (
    ...
    from (
      select
        bucket_line.fund_id,
        sum(bucket_line.debit_amount) as bucket_debits,
        sum(bucket_line.credit_amount) as bucket_credits
      from public.journal_entry_lines as bucket_line
      where bucket_line.journal_entry_id = created_journal_entry_id
      group by bucket_line.fund_id
