-- Durable user attribution for audit_events.user actors.
...
comment on column public.audit_events.actor_user_id is
  'Auth user who performed the action when actor_type = ''user''. NULL for system/integration actors and legacy user rows.';
...
comment on function public.update_compliance_item_status(uuid, uuid, text) is
  'Updates compliance_items.status for staff+ members and appends an audit_events row with actor_user_id attribution atomically. SECURITY DEFINER is the sole authenticated write path; no direct table UPDATE/INSERT RLS.';
