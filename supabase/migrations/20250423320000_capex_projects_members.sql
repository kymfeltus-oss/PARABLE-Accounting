...
CREATE TABLE parable_ledger.capex_projects (
...
CREATE INDEX idx_capex_tenant ON parable_ledger.capex_projects (tenant_id, status);
...
CREATE TABLE parable_ledger.congregation_members (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id UUID NOT NULL REFERENCES parable_ledger.tenants (id) ON DELETE CASCADE,
...
CREATE INDEX idx_congregation_tenant ON parable_ledger.congregation_members (tenant_id, created_at DESC);
